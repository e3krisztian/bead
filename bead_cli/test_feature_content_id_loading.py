...
def test_status_displays_input_information_correctly(shell, bead_a, bead_with_history, box, check, times):
    """
    ...
    bead('edit', bead_a)
    cd(bead_a)
    bead('input', 'add', 'input1', 'copied_bead1')
    bead('input', 'add', 'input2', 'copied_bead2')
...
def test_update_finds_newest_by_kind_not_name(shell, bead_a, bead_with_history, box, check, times):
    """
...
def test_explicit_bead_update_with_new_reference(shell, bead_a, bead_with_history, box, check, times):
    ...
    bead('edit', bead_a)
    cd(bead_a)
    bead('input', 'add', 'test_input', 'initial_bead')
    ...
    bead('input', 'update', 'test_input', 'newer_bead')
    check.loaded('test_input', times.TS3)

    # Update without explicit reference should find newest by kind
    bead('input', 'update', 'test_input', '--no-name')
    check.loaded('test_input', times.TS5)  # finds newest of the kind


def test_save_and_edit_preserves_content_id_references(shell, bead_a, bead_with_history, box, check, times):
    """
    ...
    bead('edit', bead_a)
    cd(bead_a)
    bead('input', 'add', 'input1', 'test_bead1')
    bead('input', 'add', 'input2', 'test_bead2')
    check.loaded('input1', times.TS1)
    check.loaded('input2', times.TS2)
...
def test_load_finds_renamed_bead_by_content_id(
    shell, bead_a, bead_b, box, beads: Dict[str, ZipArchive], check, times
):
